...
// Definimos estructura de parámetros que pasaremos a thread_fn
struct params {
  FILE *fp;
  int N;
  int **num_flights;
...
void *thread_fn(void *arg){
    ...
    char** data = (char **) malloc_matrix(info->N, MAXCHAR, sizeof(char));
    ...
    } while (its == info->N); // Iteramos siempre que hayamos leído N líneas previamente

    free_matrix((void **) data, info->N);
...
void read_airports_data(int **num_flights, char **airports, char *fname) 
{
  ...
  // Parametros para la función thread_fn()
  struct params info = {fp, N, num_flights, airports};

  /* Creamos los hilos*/
  for (i = 0; i < NUM_HILOS; i++){   
    err = pthread_create(&th[i], NULL, thread_fn, (void*) &info);
    if (err != 0) {
